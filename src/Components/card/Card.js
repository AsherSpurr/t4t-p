            ...
            <section className={locName === name ? 'LocDetails_div_container' : `Card_div_container`} tabIndex='0' onClick={() => navigate(`/locations/${name}`)} onKeyDown={(e) => e.key === 'Enter' || e.key === ' ' ? navigate(`/locations/${name}`): console.log(e.key)}>
                ...
                <div className={locName === name ? 'LocDetails_contents_container' : 'Card_contents_container'}>
                    ...
                    <div className={locName === name ? 'LocDetails_icons_container' : 'Card_icons_container'}>
                        ...
                        <p className={locName === name ? 'LocDetails_p' : 'Card_p'}>
                            {access ? <img id={locName === name ? 'LocDetails_icon_accessible' : 'Card_icon_unisex'} src={truesvg} alt='Status of true' height='20px' width='20px'/> : <img id='Card_icon_unisex' src={falsesvg} alt='Status of false' height='20px' width='20px'/>}
                            Accessible
                        </p>
                        </div>
                    </div>
