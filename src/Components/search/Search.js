        ...
        const fetchLatLonSearch = (e, street, town, state, key) => {
            ...
            .then(data => {
                if(data.status) {
                    ...
                    sessionStorage.setItem('lat', JSON.stringify(data.results[0].geometry.location.lat))
                    sessionStorage.setItem('lon', JSON.stringify(data.results[0].geometry.location.lng))
                } else {
